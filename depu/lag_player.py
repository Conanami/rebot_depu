...
def afterFlopDecision(pubnum,singleWinrate,finalWinrate,leftman,rtSit):
    ...
    if(pubnum==0):
        ...
        if(finalWinrate>=0.75 and rtSit.callchip<=rtSit.potsize/3): return (3,4)
...
def InOpenRange(myhand):
    ...
    #大的连牌和中的口袋对
    if(abs(myhand[0].num-myhand[1].num)<=2 and myhand[0].num>=5 and myhand[1].num>=5): return True
    #同花连牌
    if(abs(myhand[0].num-myhand[1].num)<=3 and myhand[0].suit==myhand[1].suit and (myhand[0].num>=5 or myhand[1].num>=5)): 
        return True
    return False
