    ...
    if(pubnum>=1 and pubnum<=3):
        if(leftman==2):
            if(rtSit.callchip==0):
                ...
                if(random.random()>=0.2 and getWaitingman(rtSit)==0): 
                    print('翻牌后位咋呼')
                    return (3,1)
        ...
        if(leftman==2):
            ...
            if(rtSit.callchip==0):
                ...
                if(rtSit.potsize>20*rtSit.bb):
                    ...
                    return (2,0)
                if(rtSit.potsize<12*rtSit.bb and random.random()>0.3): 
                    print('河牌咋呼1')
                    return (3,2)
                if(rtSit.potsize>=12*rtSit.bb and rtSit.potsize<=20*rtSit.bb and random.random()>0.5): 
                    print('河牌咋呼2')
                    return (3,1)
                return (2,0)
            #对手下了小注
